(function() {
  ...
  luacheck = function(filename, options, callback) {
    ...
    var file;
    if (options.string) {
      exec = 'echo "' + filename.toString() + '" |' + exec; // We pipe the string over to the input of luacheck since options.string tells us that filename is actually a string of lua source code.
      file = '-'; // This tells luacheck that filename is actually a string of lua source code.
    } else {
      file = path.resolve('.', filename);
    }

    args = [file, "--quiet", "--codes", "--ranges", "--no-color", "--formatter", "plain"].concat(argparse(options));
